...
Page({
  ...
  async showNewItem() {
    ...
    if (storage.all(Item)) {
      ...
      data = await myModel.showItem(State,true)
      storage.add(Item, data)
    }
    if (data) {
      this.setData({
  ...
  async getNewData(item) {
    let latest = await myModel.theLatest(this.data.index, State,true)
    let oldItem, newItem;
    try {
      oldItem = item._id;
      ...
      if (!oldItem) {
        let data = await myModel.showItem(State,true);
        this.setData({
          items: data
        })
        return;
